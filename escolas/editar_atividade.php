...
if(isset($_GET['id'])){  
    $atividadeId = $_GET['id'];
    $atividadesById = get_atividades_by_id($pdo, $atividadeId);
    $action = $_SERVER['PHP_SELF']."?id=".$_GET['id'];
}

$canEdit = false;
if($userData['is_admin'] || $atividadesById['atividade_escola_id'] == $userData['escola_id']){
    $canEdit = true;
}
...
if($_SERVER["REQUEST_METHOD"] == "POST" ){
    ...
    if(empty($errors)) {
        if($userType == 0) {
            edit_atividade($pdo, $tipo_atividade, $titulo, $data_inicio, $data_fim, $descricao, $roteiro, $userData['escola_id'], $atividadeId);
        } else if($userType == 1) {
            edit_atividade_admin($pdo, $tipo_atividade, $titulo, $data_inicio, $data_fim, $descricao, $roteiro, $escola, $atividadeId);
        }
        $_SESSION['success_message'] = 'Atividade editada com sucesso!';
        
        header("location: dashboard.php");
        exit();
    } else {
        header("location: ../editar_atividade.php?error=" . urlencode(implode(", ", $errors)));
        exit();
    }
} 
?>
...
<head>
    ...
    <title>Agrupamento de Escolas João de Deus - Editar Atividade</title>
                            ...
                            <form action="<?=$action?>" method="POST" enctype="multipart/form-data">
                                <div class="row first_row">
                                    <div class="mb-3 col-6">
                                        ...
                                        <input type="text" class="criar_input" id="titulo" name="titulo" value="<?=$atividadesById['titulo_atividade']?>">
                                    </div>
                                    <div class="mb-3 col-6">
                                        <label for="descricao" class="criar_label">Descrição</label>
                                        <textarea class="criar_input" id="descricao" name="descricao" rows="3" ><?=$atividadesById['descricao_atividade']?></textarea>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="mb-3 col-6">
                                        <label for="data-inicio" class="criar_label">Data-Inicio</label>
                                        <input type="date" class="criar_input" id="data_inicio" name="data_inicio" value="<?=$atividadesById['inicio_atividade']?>">
                                    </div>
                                    <div class="mb-3 col-6">
                                        <label for="hora" class="criar_label">Data-Fim</label>
                                        <input type="date" class="criar_input" id="data_fim" name="data_fim"  value="<?=$atividadesById['termino_atividade']?>">
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label for="tipo" class="criar_label">Tipo de Atividade</label>
                                    <select class="criar_input" id="tipo" name="tipo">
                                        <option value="" <?php echo ($atividadesById['tipo_atividade'] == "") ? "selected" : ""; ?>>Escolha o tipo de atividade</option>
                                        <option value="0" <?php echo ($atividadesById['tipo_atividade'] == "0") ? "selected" : ""; ?>>Atividade</option>
                                        <option value="1" <?php echo ($atividadesById['tipo_atividade'] == "1") ? "selected" : ""; ?>>Projeto</option>
                                        <option value="2" <?php echo ($atividadesById['tipo_atividade'] == "2") ? "selected" : ""; ?>>Clube</option>
                                        <option value="3" <?php echo ($atividadesById['tipo_atividade'] == "3") ? "selected" : ""; ?>>Visita de estudo</option> 
                                    </select>
                                </div>
                                <div class="mb-3">
                                    <label for="roteiro" class="criar_label">Roteiro</label>
                                    <input type="text" class="criar_input" id="roteiro" name="roteiro" value="<?=$atividadesById['roteiro_atividade']?>" disabled>
                                </div>
                                <?php if($userType == 1): ?>
                                    <div class="mb-3">
                                        <label for="nome_escola" class="criar_label">Escola</label>
                                        <select class="criar_input" id="nome_escola" name="nome_escola">
                                            <option <?php echo ($atividadesById['atividade_escola_id'] == '') ? 'selected' : ''; ?>>Escolha a escola</option>
                                            <option value="3" <?php echo ($atividadesById['atividade_escola_id'] == '3') ? 'selected' : ''; ?>>Escola Secundária João de Deus</option>
                                            <option value="4" <?php echo ($atividadesById['atividade_escola_id'] == '4') ? 'selected' : ''; ?>>EB 2,3 Santo António</option>
                                            <option value="5" <?php echo ($atividadesById['atividade_escola_id'] == '5') ? 'selected' : ''; ?>>Escola Básica Ria Formosa</option>
                                            <option value="6" <?php echo ($atividadesById['atividade_escola_id'] == '6') ? 'selected' : ''; ?>>EB 1 da Ferradeira</option>
                                        </select>
                                    </div>
                                <?php endif; ?>
                                <button type="submit" class="btn botao_procurar" id="btn_criar">Editar</button>
