...
function get_notice(object $pdo){
    $stmt = $pdo->prepare("SELECT * FROM tbl_noticias");
    $stmt->execute();
...
function latest_news(object $pdo){
    $stmt = $pdo->prepare("SELECT * FROM tbl_noticias ORDER BY id_noticias DESC LIMIT 3");
    $stmt->execute();
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
